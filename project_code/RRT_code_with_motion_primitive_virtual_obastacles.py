...
#from publish_data import PurePursuit 
#from collision2 import Collision_Checker

from collisions_rotated import Collision_virtual_check, Car
...
class tree:
    ...
    def motion(self, Node, primitive_dict,cc):
        ...
        car1 = Car(-2, 3, 4, 2.5, 0)	; car2 = Car(8, 3, 4, 2.5, 0)   # Parallel parking
        
        for i in range(Ed_check):
            temp=int((i+1)*ln/Ed_check)
            if Collision_virtual_check(s[temp-1, 0], s[temp-1, 1] ,s[temp-1, 2],car1, car2):
                return None, None
    ...
    def Tree_search (self,N_start, N_goal, plot_flag= False,index=0):
        ...
        if data_write_flag:
            with open('./Live_plans/Trajectory_test_plan_virtual_'+ str(index), 'wb') as fp: ########## Note data is in reverse from goal
...
if __name__ == "__main__":
    Tr=tree()  
    N_start=node(None, 0, 0 ,0, 0)
    ################## Goals
    ##### node(None, 4, 3, np.pi/2, 0) perpendicular parking
    ##### node(None, 4, 3, np.pi/2, 0) perpendicular parking
    
    
    N_goal=node(None, 4, 3,0, 0)
    Tr.add_n(N_start)
    ...
    for i in range(1000):
        if random.uniform(0,1)<p_g:
            ...
            x=random.uniform(-5,5)           ####################expansion in state space x
            y=random.uniform(-5,5)          ####################expansion in state space y
            th=random.uniform(-0.5*np.pi,0.5*np.pi)
            v=random.uniform(-0.5,0.5)
            N,_=Tr.nearest_neigh(x,y,th,v)

    
        [N_new,E_new]=Tr.motion(N, primitive_dict,3)#,cc)############################ For Map need to ensure
